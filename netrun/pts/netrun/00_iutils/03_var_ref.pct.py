...
def _cast_var_value(raw: str, target_type: type, var_name: str = "") -> Any:
    """Cast a raw string value to the target Python type.
    ...
    try:
        ...
        raise ValueError(
            f"Cannot cast var '{var_name}' value {raw!r} to {target_type.__name__ if hasattr(target_type, '__name__') else target_type}: {exc}"
        ) from exc
...
def _resolve_env_vars_in_list(lst: list) -> tuple[list, bool]:
    """Recursively resolve EnvVar instances in a list of VarResolvableModel items."""
    changed = False
    new_list = []
    for item in lst:
        if isinstance(item, VarResolvableModel):
...
def _resolve_env_vars_in_dict(d: dict) -> tuple[dict, bool]:
    """Recursively resolve EnvVar instances in a dict of VarResolvableModel values."""
    changed = False
    new_dict = {}
    for key, value in d.items():
        if isinstance(value, VarResolvableModel):
...
def _resolve_var_ref_value(ref: "VarRef", resolved_vars: dict[str, Any], target_type: type, field_context: str) -> Any:
    ...
    if ref.var in resolved_vars:
        ...
        if isinstance(value, str) and target_type is not str and target_type is not Any:
            return _cast_var_value(value, target_type, var_name=f"$var:{ref.var}")
        return value
...
def _resolve_var_refs_in_list(lst: list, resolved_vars: dict[str, Any]) -> tuple[list, bool]:
    """Recursively resolve $var VarRef instances in a list of VarResolvableModel items."""
    changed = False
    new_list = []
    for item in lst:
        if isinstance(item, VarResolvableModel):
...
def _resolve_var_refs_in_dict(d: dict, resolved_vars: dict[str, Any]) -> tuple[dict, bool]:
    """Recursively resolve $var VarRef instances in a dict of VarResolvableModel values."""
    changed = False
    new_dict = {}
    for key, value in d.items():
        if isinstance(value, VarResolvableModel):
...
def _resolve_project_root_paths_in_list(lst: list, project_root: Path) -> tuple[list, bool]:
    """Recursively resolve project root paths in a list of VarResolvableModel items."""
    changed = False
    new_list = []
    for item in lst:
        if isinstance(item, VarResolvableModel):
...
def _resolve_project_root_paths_in_dict(d: dict, project_root: Path) -> tuple[dict, bool]:
    """Recursively resolve project root paths in a dict of VarResolvableModel values."""
    changed = False
    new_dict = {}
    for key, value in d.items():
        if isinstance(value, VarResolvableModel):
...
class VarResolvableModel(BaseModel):
    """Base class for config models that may contain VarRef fields."""

    def resolve_env_vars(self) -> "VarResolvableModel":
        ...
        for field_name, field_info in type(self).model_fields.items():
            ...
            if isinstance(value, VarRef) and value.env is not None:
                ...
                if raw is None:
                    ...
                    updates[field_name] = _cast_var_value(raw, target_type, var_name=value.env)
    ...
    def resolve_var_refs(self, resolved_vars: dict[str, Any]) -> "VarResolvableModel":
    ...
    def resolve_project_root_paths(self, project_root: Path) -> "VarResolvableModel":
        ...
        Recurses into nested ``VarResolvableModel``, list, and dict values.
