...
#|export
@dataclass
class EffectiveCacheConfig:
    """Resolved cache config for a node (net defaults merged with node overrides)."""
    cache_what: CacheWhat
    hash_method: HashMethod
    pickling_method: PicklingMethod
    pickling_args: dict
    evaluate_lazy_value_for_cache: bool
...
# %%
#|export
_EPOCH_COUNT_KEY = "__epoch_count__"
...
class NodeCacheStore:
    ...
    def __init__(
        ...
        # Restore epoch count from persistent storage
        self._epoch_count = self._disk_cache.get(_EPOCH_COUNT_KEY, 0)
    ...
    def store(self, data: CachedEpochData) -> None:
        ...
        self._epoch_count += 1
        self._disk_cache.set(_EPOCH_COUNT_KEY, self._epoch_count)
        ...
        if self._epoch_count <= self._sample_size:
            ...
            if j <= self._sample_size:
                existing_keys = [k for k in self._disk_cache if k != _EPOCH_COUNT_KEY]
                if existing_keys:
    ...
    def entry_count(self) -> int:
        """Number of entries in the cache (excluding internal metadata)."""
        count = len(self._disk_cache)
        if _EPOCH_COUNT_KEY in self._disk_cache:
            count -= 1
        return count
...
class NetCacheStore:
    ...
    def get_effective_config(self, node_name: str) -> EffectiveCacheConfig | None:
        """Get merged config for a node (node overrides net).

        Returns None if no net-level cache config exists.
        """
        if self._net_config is None:
            return None
        ...
        evaluate_lazy = self._net_config.evaluate_lazy_value_for_cache
