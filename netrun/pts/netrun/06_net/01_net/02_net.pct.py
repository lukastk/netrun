...
from netrun.net.config import NetConfig, NodeExecutionConfig, PortConfig, RemotePoolConfig, SignalValue, EpochSignalValue, EpochFailedSignalValue, signal_port_name, is_signal_port, is_control_port, control_type_from_port, is_control_salvo_condition, control_port_name, CONTROL_TYPES
from netrun.net.config._nodes import resolve_effective_exec_field
...
#|export
class NetCacheAPI:
    """Cache inspection and management API. Accessed via net.cache."""

    def __init__(self, net: "Net"):
        self._net = net

    def entries(self, node_name: str) -> list:
        """All cached entries for a node."""
        return self._net._cache_store.get_entries(node_name)

    def input_salvos(self, node_name: str) -> list[dict[str, list]]:
        """Input salvos from all cached entries."""
        return [e.input_values_by_port for e in self._net._cache_store.get_entries(node_name)]

    def output_salvos(self, node_name: str) -> list[dict[str, list]]:
        """Output salvos from all cached entries."""
        return [e.output_port_values for e in self._net._cache_store.get_entries(node_name)]

    def output_for_input(self, node_name: str, input_values: dict[str, list]) -> Any:
        """Look up cached output for specific input values."""
        if not self._net._cache_store.is_cache_enabled(node_name):
            return None
        from netrun.packets import PacketStore
        ...
        input_hash = self._net._cache_store.hash_input_salvo(node_name, packets, temp_store)
        return self._net._cache_store.lookup(node_name, input_hash)

    def stats(self, node_name: str | None = None) -> dict:
        """Cache statistics. If node_name given, returns stats for that node only."""
        all_stats = self._net._cache_store.cache_stats()
        if node_name is not None:
            return all_stats.get(node_name, {"entry_count": 0, "epoch_count": 0, "sample_size": None})
        return all_stats

    def is_enabled(self, node_name: str) -> bool:
        """Whether caching is enabled for a node."""
        return self._net._cache_store.is_cache_enabled(node_name)

    def clear(self, node_name: str | None = None) -> None:
        """Clear cache. If node_name given, clears only that node."""
        if node_name is not None:
            self._net._cache_store.clear_node(node_name)
        else:
            self._net._cache_store.clear_all()

    def clear_for_input(self, node_name: str, input_values: dict[str, list]) -> None:
        """Clear cached output for a specific input salvo."""
        from netrun.packets import PacketStore
        temp_store = PacketStore()
        packets: dict[str, list[str]] = {}
        for port_name in sorted(input_values.keys()):
            ...
            packets[port_name] = pkt_ids
        input_hash = self._net._cache_store.hash_input_salvo(node_name, packets, temp_store)
        self._net._cache_store.clear_output_for_input(node_name, input_hash)


class NetLogQuery:
    """Log query and printing API. Accessed via net.logs."""

    def __init__(self, net: "Net"):
        self._net = net

    def for_epoch(self, epoch_id: str) -> list[tuple[datetime, str]]:
    ...
    def for_node(self, node_name: str) -> list[tuple[datetime, str]]:
        """Get all print output for a node across all epochs."""
        logs = []
        for record in self._net._epochs.values():
            if record.node_name == node_name:
                logs.extend(record.logs)
        return logs

    def all_chronological(self) -> list[tuple[datetime, str, str, str]]:
        """All logs sorted by timestamp."""
        all_logs = []
        for epoch_id, record in self._net._epochs.items():
    ...
    def print_epoch(self, epoch_id: str, include_timestamps: bool = True) -> None:
        """Print logs for one epoch."""
        record = self._net._epochs.get(epoch_id)
        if record is None:
            return
        for timestamp, message in record.logs:
    ...
    def print_node(self, node_name: str, include_timestamps: bool = True, chronological: bool = False) -> None:
        """Print all logs for a node."""
        if chronological:
            for record in self._net._epochs.values():
                if record.node_name != node_name:
                    continue
    ...
    def print_all(self, include_timestamps: bool = True, chronological: bool = False) -> None:
        """Print all logs."""
        if chronological:
            logs = self.all_chronological()
...
class Net:
    ...
    def __init__(self, config: NetConfig, run_init_nodes: bool = True):
        ...
        # Sub-objects for cache and log access
        self.cache = NetCacheAPI(self)
        self.logs = NetLogQuery(self)
    ...
    def _get_edges_from_port(self, node_name: str, port_name: str) -> list:
    ...
    def _has_downstream_connection(self, node_name: str, port_name: str) -> bool:
        ...
        """
        return len(self._get_edges_from_port(node_name, port_name)) > 0

    # --- Packet Creation & Injection API ---

    def _create_external_packet(self, value: Any) -> str:
        """Create a packet outside the network with a value.

        The packet is created in the OutsideNet location and can be
        transported to input ports using _inject_packet().
    ...
    def _create_external_packets(self, values: list[Any]) -> list[str]:
        ...
        packet_ids = []
        for value in values:
            packet_id = self._create_external_packet(value)
            packet_ids.append(packet_id)
        return packet_ids

    def _inject_packet(self, packet_id: str, node_name: str, port_name: str) -> None:
    ...
    def inject_data(self, node_name: str, port_name: str, values: list[Any]) -> list[str]:
        ...
        This is a convenience method that combines _create_external_packets()
        and _inject_packet() into a single call.
        ...
        """
        packet_ids = self._create_external_packets(values)
        for packet_id in packet_ids:
            self._inject_packet(packet_id, node_name, port_name)
        return packet_ids
    ...
    def get_node_config(self, node_name: str):
        """Get the resolved NodeConfig for a node by name.

        Returns:
            The NodeConfig for the node.
        Raises:
            KeyError: If node_name not found.
        """
        for node_config in self._config_resolved.graph.nodes:
            if node_config.name == node_name:
                return node_config
        raise KeyError(f"Node '{node_name}' not found in graph")

    def get_packets_at_port(self, node_name: str, port_name: str) -> list:
        """Get packets waiting at a node's input port.

        Returns:
            List of Packet objects from netrun-sim.
        """
        location = netrun_sim.PacketLocation.input_port(node_name, port_name)
        packet_ids = self._netsim.get_packets_at_location(location)
        return [self._netsim.get_packet(pid) for pid in packet_ids]

    def get_packets_at_all_ports(self, node_name: str) -> dict[str, list]:
        """Get all packets at all input ports of a node.
        ...
        for port_name in (node_config.in_ports or {}):
            result[port_name] = self.get_packets_at_port(node_name, port_name)
        return result

    def get_packets_in_epoch(self, epoch_id: str) -> list:
    ...
    def _emit_out_of_epoch_signal(self, node_name: str, signal_type: str, epoch_id: str | None = None):
        ...
        edges = self._get_edges_from_port(node_name, port_name)
        if edges:
            self._do_action(
                netrun_sim.NetAction.transport_packet_to_location(
    ...
    def on_epoch_start(self, callback: Callable, *, node: str | None = None) -> Callable[[], None]:
        """Register a callback that fires when an epoch starts.
        ...
        Args:
            ...
            node: If set, only fire for this node. None means all nodes.

        Returns:
            A callable that removes the callback when called.
        """
        return self._register_epoch_callback("start", callback, node_name=node)

    def on_epoch_end(self, callback: Callable, *, node: str | None = None) -> Callable[[], None]:
        """Register a callback that fires when an epoch ends.
        ...
        Returns:
            A callable that removes the callback when called.
        """
        return self._register_epoch_callback("end", callback, node_name=node)
