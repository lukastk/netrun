<script lang="ts">
	...
	interface Props {
		...
		/** Called on double-click (open subgraph tab) */
		onSubgraphOpen?: (event: { nodeId: string; data: SubgraphNodeData }) => void;
		/** Called when inline expand/collapse is toggled */
		onSubgraphToggleExpand?: (event: { nodeId: string }) => void;
	...
	function handleDoubleClick() {
		onSubgraphOpen?.({ nodeId: id, data });
	}

	function handleToggleExpand(e: MouseEvent) {
		e.stopPropagation();
		onSubgraphToggleExpand?.({ nodeId: id });
	}
...
>
	...
	<div bind:this={prePortsEl}>
		...
		{#if data.description && !hideDescription}
			<!-- svelte-ignore a11y_no_static_element_interactions -->
			<div class="node-description" style:cursor={selected ? 'pointer' : undefined} onclick={selected ? (e: MouseEvent) => { e.stopPropagation(); handleDescToggle(); } : undefined}>
				<!-- svelte-ignore a11y_no_static_element_interactions -->
	...
	{:else}
		...
		<div class="expanded-ports" style:padding-top="{portsPaddingTop}px">
			<PortList nodeId={id} ports={data.inPorts} side="in" {portGroupStates} {hidePortNames}
				exposedPortNames={exposedInPortNames} {signalConfig} {controlConfig} {onPortGroupToggle} />
			<PortList nodeId={id} ports={data.outPorts} side="out" {portGroupStates} {hidePortNames}
				exposedPortNames={exposedOutPortNames} {signalConfig} {controlConfig} {onPortGroupToggle} />
		</div>
