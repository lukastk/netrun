...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add(
        "PacketNotFoundError",
        m.py().get_type::<PacketNotFoundError>(),
    )?;
    m.add(
        "EpochNotFoundError",
        m.py().get_type::<EpochNotFoundError>(),
    )?;
    m.add(
        "EpochNotRunningError",
        m.py().get_type::<EpochNotRunningError>(),
    )?;
    m.add(
        "EpochNotStartableError",
        m.py().get_type::<EpochNotStartableError>(),
    )?;
    m.add(
        "CannotFinishNonEmptyEpochError",
        m.py().get_type::<CannotFinishNonEmptyEpochError>(),
    )?;
    m.add(
    ...
    m.add(
        ...
        m.py().get_type::<OutputPortNotFoundError>(),
    )?;
    m.add(
        "OutputPortFullError",
        m.py().get_type::<OutputPortFullError>(),
    )?;
    m.add(
        "SalvoConditionNotFoundError",
        m.py().get_type::<SalvoConditionNotFoundError>(),
    )?;
    m.add(
        "SalvoConditionNotMetError",
        m.py().get_type::<SalvoConditionNotMetError>(),
    )?;
    m.add(
        "MaxSalvosExceededError",
        m.py().get_type::<MaxSalvosExceededError>(),
    )?;
    m.add("NodeNotFoundError", m.py().get_type::<NodeNotFoundError>())?;
    m.add(
        "PacketNotAtInputPortError",
        m.py().get_type::<PacketNotAtInputPortError>(),
    )?;
    m.add(
        "InputPortNotFoundError",
        m.py().get_type::<InputPortNotFoundError>(),
    )?;
    m.add(
        "InputPortFullError",
        m.py().get_type::<InputPortFullError>(),
    )?;
    m.add(
        "CannotMovePacketFromRunningEpochError",
        m.py().get_type::<CannotMovePacketFromRunningEpochError>(),
    )?;
    m.add(
        "CannotMovePacketIntoRunningEpochError",
...
pub fn net_action_error_to_py_err(err: netrun_sim::net::NetActionError) -> PyErr {
    ...
    match err {
        ...
        NetActionError::UnsentOutputSalvo {
            epoch_id,
            port_name,
        } => UnsentOutputSalvoError::new_err(format!(
        ...
        NetActionError::OutputSalvoConditionNotFound {
            epoch_id,
            condition_name,
        ...
        NetActionError::MaxOutputSalvosReached {
            epoch_id,
            condition_name,
        ...
        NetActionError::SalvoConditionNotMet {
            epoch_id,
            condition_name,
        ...
        NetActionError::PacketNotAtInputPort {
            packet_id,
            port_name,
            node_name,
        ...
        NetActionError::InputPortNotFound {
            node_name,
            port_name,
