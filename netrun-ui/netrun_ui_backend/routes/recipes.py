    ...
    if hasattr(module, 'get_prompts'):
        try:
            ...
            if not isinstance(prompts, list):
                raise HTTPException(status_code=400, detail="get_prompts() must return a list")
            return GetPromptsResponse(prompts=prompts)
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"Error calling get_prompts(): {e}")
...
async def execute(request: ExecuteRequest) -> ExecuteResponse:
    ...
    if not hasattr(module, 'run'):
        raise HTTPException(status_code=400, detail="Recipe missing required 'run' function")

    try:
        result = module.run(request.config, request.inputs)
        if not isinstance(result, dict):
            raise HTTPException(status_code=400, detail="run() must return a dict")
